...
ENV PATH=${PATH}:${JAVA_HOME}/bin LD_LIBRARY_PATH=${HADOOP_HOME}/lib/native:/usr/local/lib:${LD_LIBRARY_PATH} HADOOP_CONF_DIR=${HADOOP_HOME}/etc/hadoop

# install and configure hadoop
COPY Configs/hadoop-3.1.2-src.tar.gz /
RUN tar -xvf hadoop-3.1.2-src.tar.gz
RUN cd hadoop-3.1.2-src && mvn package -Pdist,native -DskipTests -Dtar
RUN mv hadoop-3.1.2-src/hadoop-3.1.2-src/hadoop-dist/target/hadoop-3.1.2 ${HADOOP_HOME}
